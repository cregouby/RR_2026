...
- Déclarer explicitement les en-têtes de colonnes et de lignes (balise
  `<th>`). 
...
Les composants web générés par Shiny produisent déjà un HTML correct et
étiqueté.\
L’accessibilité consiste surtout à

 -  **ne pas casser** la base existante,
 -   à ajouter délibérément quelques éléments ARIA **là où c’est vraiment nécessaire**,
 -   l’utilisabilité au clavier pour tout ce qui est personnalisé
 -   un contraste suffisant et du texte alternatif.
...
``` python
In [9]: from delineo import segment_rfdetr

In [11]: 
    ...:     segment_rfdetr(
    ...:         input_dir=here("data/drawings/),
    ...:         checkpoint_path=[27],
    ...:         file_glob="* - 1 _ *",
...
Cell In[11], line 1
----> 1 segment_rfdetr(
      2     input_dir=here("data/drawings/),
      3     checkpoint_path=[27],
      4     file_glob="* - 1 _ *",
      5     output_parquet=here("data/log/x.parquet")
      6 )

File ~/_AA/delineo/delineo/ml/drawing_segmentation_model.py:191, in segment_rfdetr(input_dir, checkpoint_path, output_parquet, min_confidence, nms_iou, file_glob)
    167 def segment_rfdetr(
...
- Navigabilité des boutons de la barre de navigation , de la barre
  verticale, et des modes dark/light
